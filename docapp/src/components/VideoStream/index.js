...
class VideoStream extends Component {
    ...
    captureFrame = () => {
        ...
        d = this.canvasContext.getImageData(0, 0, this.streamWidth, this.streamHeight)
        console.log(d)
        // return d;
        this.props.onCapture(d)
    }
    ...
    render() {
        return (
            <>
                <video style={{ ...videoStyles, ...this.props.style }} ref={v => (this.video = v)} id={ this.props.id }/>
                <button type="button" onClick={this.captureFrame}>Capture</button>
            </>
        )
    }
};
